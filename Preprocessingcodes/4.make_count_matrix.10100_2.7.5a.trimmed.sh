...
cat ${array[0]}_counts.txt | awk '{print $1}' > count_matrix_10100_trimmed.txt

for item in ${array[*]}
do
    # read in second column of each file and add it is a new column
    echo $item
    eval "cat '$item'_counts.txt | awk '{print \$2}' | paste count_matrix_10100_trimmed.txt - > output.txt"
mv output.txt count_matrix_10100_trimmed.txt
done

# delete last 5 lines
head -n -5 count_matrix_10100_trimmed.txt > tmp.txt && mv tmp.txt count_matrix_10100_trimmed.txt

# add header
output=$(printf "\t%s" "${array[@]}")
echo -e "Name${output}" | cat - count_matrix_10100_trimmed.txt > tmp.txt && mv tmp.txt count_matrix_10100_trimmed.txt
